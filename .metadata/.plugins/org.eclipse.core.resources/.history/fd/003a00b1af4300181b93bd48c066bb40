...
public class ParameterRestService {
	...
	private String maakMeting(String token, String type, String projID, Meting2 m, String experimentNaam, int hertz) {
		...
		if (claims != null || type.equals("toevoegenViaToken")) {
			...
			String b = m.getX();
			int spatie = 0;
			for (int i = 0; i < m.getX().length(); i++) {
				if (b.charAt(i) == ' ') {
					x.add(Float.parseFloat(b.substring(spatie + 1, i - 2)));
					spatie = i;
				} else if (b.charAt(i) == ']') {
					x.add(Float.parseFloat(b.substring(spatie + 1, i - 1)));
					spatie = i;
			...
			for (int i = 0; i < b.length(); i++) {
				if (b.charAt(i) == ' ') {
					y.add(Float.parseFloat(b.substring(spatie + 1, i - 2)));
					spatie = i;
				} else if (b.charAt(i) == ']') {
					y.add(Float.parseFloat(b.substring(spatie + 1, i - 1)));
					spatie = i;
			...
			for (int i = 0; i < b.length(); i++) {
				if (b.charAt(i) == ' ') {
					z.add(Float.parseFloat(b.substring(spatie + 1, i - 2)));
					spatie = i;
				} else if (b.charAt(i) == ']') {
					z.add(Float.parseFloat(b.substring(spatie + 1, i - 1)));
					spatie = i;
